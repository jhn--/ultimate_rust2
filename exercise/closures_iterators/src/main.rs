...
fn main() {
    ...
    pairs
        ...
        .map(|t| (t.0 + 1, t.1)) // one way of doing it
        // .map(|(x, y)| (x + 1, y)) // another way, by destructuring
    ...
    let transformed: Vec<_> = words
        .into_iter()
        .filter(|w| !w.contains('h')) // filters away words that doesn't containt 'h'
        .map(|w| w.to_uppercase()) // change the remaining words to uppercase
        .collect();
    ...
    // collect() is the iterator consumer
    println!(
        "{:?}",
        numbers.iter_mut().map(|x| *x * 3).collect::<Vec<_>>()
    );

    // also works
    let numbers2: Vec<_> = numbers.iter_mut().map(|x| *x * 3).collect();
    println!("{:?}", numbers2);
}
